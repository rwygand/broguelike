...
impl TownBuilder {
    ...
    fn town_walls(&mut self, rng: &mut RandomNumberGenerator, build_data : &mut BuilderMap)
                  -> (HashSet<usize>, i32)
    {
                 ...
                 doors : &[usize])
    {
        ...
        for door_idx in doors.iter() {
            ...
            let door_pt = Point::new( *door_idx as i32 % build_data.map.width as i32, *door_idx as i32 / build_data.map.width as i32 );
            for r in roads.iter() {
                nearest_roads.push((
                    *r,
