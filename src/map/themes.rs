...
fn get_forest_glyph(idx:usize, map: &Map) -> (FontCharType, RGB, RGB) {
    ...
    match map.tiles[idx] {
        TileType::Wall => { glyph = to_cp437('♣'); fg = RGB::from_f32(0.0, 0.6, 0.0); }
        TileType::Bridge => { glyph = to_cp437('.'); fg = RGB::named(CHOCOLATE); }
        TileType::Road => { glyph = to_cp437('≡'); fg = RGB::named(YELLOW); }
        ...
        TileType::UpStairs => { glyph = to_cp437('<'); fg = RGB::from_f32(0., 1.0, 1.0); }
        _ => { glyph = to_cp437('"'); fg = RGB::from_f32(0.0, 0.6, 0.0); }
    }
...
fn get_limestone_cavern_glyph(idx:usize, map: &Map) -> (FontCharType, RGB, RGB) {
    ...
    match map.tiles[idx] {
        TileType::Wall => { glyph = to_cp437('▒'); fg = RGB::from_f32(0.7, 0.7, 0.7); }
        TileType::Bridge => { glyph = to_cp437('.'); fg = RGB::named(CHOCOLATE); }
        TileType::Road => { glyph = to_cp437('≡'); fg = RGB::named(YELLOW); }
        TileType::Grass => { glyph = to_cp437('"'); fg = RGB::named(GREEN); }
        TileType::ShallowWater => { glyph = to_cp437('░'); fg = RGB::named(CYAN); }
        TileType::DeepWater => { glyph = to_cp437('▓'); fg = RGB::from_f32(0.2, 0.2, 1.0); }
        TileType::Gravel => { glyph = to_cp437(';'); fg = RGB::from_f32(0.5, 0.5, 0.5); }
        TileType::DownStairs => { glyph = to_cp437('>'); fg = RGB::from_f32(0., 1.0, 1.0); }
        TileType::UpStairs => { glyph = to_cp437('<'); fg = RGB::from_f32(0., 1.0, 1.0); }
        TileType::Stalactite => { glyph = to_cp437('╨'); fg = RGB::from_f32(0.7, 0.7, 0.7); }
        TileType::Stalagmite => { glyph = to_cp437('╥'); fg = RGB::from_f32(0.7, 0.7, 0.7); }
        _ => { glyph = to_cp437('\''); fg = RGB::from_f32(0.4, 0.4, 0.4); }
    }
...
fn get_tile_glyph_default(idx: usize, map : &Map) -> (FontCharType, RGB, RGB) {
    ...
    match map.tiles[idx] {
        ...
        TileType::DeepWater => { glyph = to_cp437('~'); fg = RGB::named(BLUE); }
        TileType::Gravel => { glyph = to_cp437(';'); fg = RGB::from_f32(0.5, 0.5, 0.5); }
        TileType::Stalactite => { glyph = to_cp437('╨'); fg = RGB::from_f32(0.5, 0.5, 0.5); }
        TileType::Stalagmite => { glyph = to_cp437('╥'); fg = RGB::from_f32(0.5, 0.5, 0.5); }
    }
