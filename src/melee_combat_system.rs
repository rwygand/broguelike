use specs::prelude::*;
use super::{WantsToMelee, Name, SufferDamage, gamelog::GameLog,
    particle_system::ParticleBuilder, Position, HungerClock, HungerState};
use bracket_lib::prelude::*;
use crate::{Attributes, Pools, Skill, Skills};
        ...
        for (entity, wants_melee, name, attacker_attributes, attacker_skills, attacker_pools) in (&entities, &wants_melee, &names, &attributes, &skills, &pools).join() {
            ...
            if attacker_pools.hit_points.current > 0 && target_pools.hit_points.current > 0 {
                ...
                if natural_roll != 1 && (natural_roll == 20 || modified_hit_roll > armor_class) {
                    ...
                    if let Some(pos) = positions.get(wants_melee.target) {
                        particle_builder.request(pos.x, pos.y, RGB::named(BLUE), RGB::named(BLACK), to_cp437('‼'), 200.0);
                    }
                } else {
                    // Miss
                    log.entries.push(format!("{} attacks {}, but can't connect.", name.name, target_name.name));
                    if let Some(pos) = positions.get(wants_melee.target) {
                        particle_builder.request(pos.x, pos.y, RGB::named(CYAN), RGB::named(BLACK), to_cp437('‼'), 200.0);
                    }
                }
            }
        }
